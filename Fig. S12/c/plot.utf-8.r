...
dat <- read.table('taxa_abund_phylum.txt', row.names = 1, sep = '\t', head = TRUE, check.names = FALSE)
...
grp <- group[2]
group_col <- c('#000000','#003399', '#CC3333')
names(group_col) <- c('1', '2','3')
group_name <- c('OS', 'SF', 'SW')
...
phylum_color <- c('#8DD3C7', '#FFFFB3', '#BEBADA', '#FB8072', '#80B1D3', '#FDB462', '#B3DE69', '#FCCDE5', '#BC80BD',  'gray')
...
mtext('Higher 1.5% (on average) phylums', side = 3, line = 1, cex = 1)
